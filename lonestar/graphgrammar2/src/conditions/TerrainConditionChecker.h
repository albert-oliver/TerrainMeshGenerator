...
private:
    ...
    bool inside_condition(const vector<Coordinates> &verticesCoords) { //TODO: Find better point location algorithm
        double lowest_x = verticesCoords[0].getX() < verticesCoords[1].getX() ? verticesCoords[0].getX()
                                                                              : verticesCoords[1].getX();
        lowest_x = verticesCoords[2].getX() < lowest_x ? verticesCoords[2].getX() : lowest_x;
        double highest_x = verticesCoords[0].getX() > verticesCoords[1].getX() ? verticesCoords[0].getX()
                                                                               : verticesCoords[1].getX();
        highest_x = verticesCoords[2].getX() > highest_x ? verticesCoords[2].getX() : highest_x;
        double lowest_y = verticesCoords[0].getY() < verticesCoords[1].getY() ? verticesCoords[0].getY()
                                                                              : verticesCoords[1].getY();
        lowest_y = verticesCoords[2].getY() < lowest_y ? verticesCoords[2].getY() : lowest_y;
        double highest_y = verticesCoords[0].getY() > verticesCoords[1].getY() ? verticesCoords[0].getY()
                                                                               : verticesCoords[1].getY();
        highest_y = verticesCoords[2].getY() > highest_y ? verticesCoords[2].getY() : highest_y;
    ...
    void compute_barycentric_coords(double *barycentric_coords, Coordinates &point,
    ...
    double get_area(const Coordinates &a, const Coordinates &b, const Coordinates &c) {
        return 0.5 *
               fabs((b.getX() - a.getX()) * (c.getY() - a.getY()) - (b.getY() - a.getY()) * (c.getX() - a.getX()));
    }
